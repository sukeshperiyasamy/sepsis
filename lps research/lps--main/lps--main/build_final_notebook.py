...
Full scientific audit + upgrade of LPS_Composite_Raman_Analysis.ipynb.
Adds: scaled/unscaled comparison, residual analysis, region-wise metrics,
adaptive peak detection, publication annotations, robust error handling.
"""
import json, pathlib

NB = pathlib.Path("LPS_Composite_Raman_Analysis.ipynb")
...
# CELL 0 — Title
# ─────────────────────────────────────────────────────────────────────────────
cells.append(md("c00-title",
"# LPS Composite Raman Analysis\n",
"### Scientifically Validated — Publication-Ready\n",
"\n",
"---\n",
"\n",
"## Scientific Context\n",
"This notebook models the **Raman fingerprint of Lipopolysaccharide (LPS)** as a weighted\n",
"superposition of five constituent fragment spectra computed by Density Functional Theory (DFT).\n",
"One experimental LPS Raman spectrum is used as the fitting target.\n",
"\n",
"### DFT Computational Method\n",
"| Parameter | Value |\n",
"|---|---|\n",
"| Software | Gaussian 16 (or Gaussian 09) |\n",
"| Method | B3LYP (Becke 3-parameter, Lee-Yang-Parr) |\n",
"| Basis set | 6-31G(d) (Pople split-valence + d-polarization) |\n",
"| Job type | `freq=Raman` — harmonic frequencies + Raman activities |\n",
"| Output | Wavenumbers (cm-1), Raman scattering activities (A4/amu) |\n",
"\n",
"### Frequency Scale Factor\n",
"DFT harmonic frequencies systematically overestimate observed frequencies due to:\n",
"anharmonicity, basis-set incompleteness, and electron correlation truncation.\n",
"For **B3LYP/6-31G(d)** the recommended scale factor is **0.9613**\n",
"(NIST Computational Chemistry Comparison and Benchmark Database, CCCBDB;\n",
"Scott & Radom, *J. Phys. Chem.* **100**, 16502, 1996).\n",
"\n",
"### Preprocessing Logic (Asymmetric by Design)\n",
"| Step | Simulated DFT | Experimental |\n",
"|---|---|---|\n",
"| Frequency scaling x0.9613 | YES | No (direct measurement) |\n",
"| Gaussian broadening FWHM=20 cm-1 | YES | No |\n",
"| ALS baseline correction | **NO** (no fluorescence bg) | YES |\n",
"| Savitzky-Golay smoothing | No (already smooth) | YES |\n",
"| Min-max normalization | YES | YES |\n",
"\n",
"### Optimization\n",
"Non-Negative Least Squares (NNLS; Lawson & Hanson, 1974) minimizes\n",
"`||I_exp - B*w||^2` subject to `w >= 0`, then weights are rescaled to sum=1.\n",
"NNLS guarantees global optimum for this convex quadratic program.\n",
))

# ─────────────────────────────────────────────────────────────────────────────
# CELL 1 — Imports
# ─────────────────────────────────────────────────────────────────────────────
cells.append(code("c01-imports",
"import warnings, sys\n",
...
# ─────────────────────────────────────────────────────────────────────────────
cells.append(md("c02-md-config","## Step 1: Configuration\n",
"All scientifically significant parameters are centralized here with full documentation.\n"))

cells.append(code("c03-config",
"# ── Workspace paths ───────────────────────────────────────────────────────────\n",
...
# ─────────────────────────────────────────────────────────────────────────────
cells.append(md("c04-md-load","## Step 2: Data Loading\n",
"Robust Excel parser handles variable headers, blank rows, multi-sheet files.\n",
"Automatically finds (Raman shift, intensity) column pair by name-scoring.\n"))

cells.append(code("c05-load-helpers",
...
cells.append(code("c06-load-files",
"datasets = {}\n",
"load_report = []\n",
"\n",
...
# ─────────────────────────────────────────────────────────────────────────────
# CELL 7-9 — Preprocessing
# ─────────────────────────────────────────────────────────────────────────────
cells.append(md("c07-md-preproc",
"## Step 3: Preprocessing Pipeline\n",
"\n",
"The pipeline is **intentionally asymmetric** — different steps for simulated vs. experimental:\n",
"\n",
"**Simulated (DFT):** Frequency scaling -> Gaussian broadening -> normalization\n",
"- Frequency scaling corrects DFT overestimation (NIST CCCBDB factor 0.9613)\n",
"- Gaussian broadening converts discrete stick spectrum to a continuum\n",
"- **ALS is NOT applied** — DFT spectra have zero fluorescence background\n",
"- **SG smoothing is NOT applied** — Gaussian broadening already produces smooth spectra\n",
"\n",
"**Experimental:** ALS baseline -> SG smoothing -> normalization\n",
"- ALS removes fluorescence background and instrumental drift\n",
"- SG smoothing reduces CCD shot noise without distorting peak positions\n",
"- No frequency scaling (experimental values are directly measured)\n",
))

cells.append(code("c08-preproc-fns",
...
cells.append(code("c09-preproc-pipeline",
"processed_spectra = {}\n",
"proc_summary = []\n",
"\n",
...
# ─────────────────────────────────────────────────────────────────────────────
# CELL 10 — Preprocessing Visualization
# ─────────────────────────────────────────────────────────────────────────────
cells.append(code("c10-preproc-plot",
...
# ─────────────────────────────────────────────────────────────────────────────
# CELL 11 — Scaled vs Unscaled comparison
# ─────────────────────────────────────────────────────────────────────────────
cells.append(code("c11-scaled-vs-unscaled",
...
# ─────────────────────────────────────────────────────────────────────────────
# CELL 12-13 — Common Grid
# ─────────────────────────────────────────────────────────────────────────────
cells.append(md("c12-md-grid",
"## Step 4: Common Raman Grid\n",
"\n",
"The shared grid is restricted to the **actual experimental measurement range**.\n",
"This is critical: extending the grid beyond the experimental data would require\n",
"interpolating zero-values where no signal exists, biasing the NNLS optimization\n",
"toward minimizing a spurious mismatch in unmeasured regions.\n",
"\n",
"All spectra (5 simulated + 1 experimental) are interpolated onto this common grid.\n",
))

cells.append(code("c13-common-grid",
...
# CELL 14-15 — Slide 1
# ─────────────────────────────────────────────────────────────────────────────
cells.append(md("c14-md-s1","## Slide 1: Individual LPS Fragment Spectra (Simulated)\n"))

cells.append(code("c15-slide1",
"# Combined overlay\n",
...
# CELL 16-17 — Slide 2
# ─────────────────────────────────────────────────────────────────────────────
cells.append(md("c16-md-s2",
"## Slide 2: Equal-Weight Composite (Null/Baseline Model)\n",
"I_LPS = (I1 + I2 + I3 + I4 + I5) / 5\n",
"Serves as a chemically unweighted reference only — the NNLS model in Slide 3 is evidence-based.\n"))

cells.append(code("c17-slide2",
...
# ─────────────────────────────────────────────────────────────────────────────
# CELL 18-19 — NNLS Optimization + Slides 3-4
# ─────────────────────────────────────────────────────────────────────────────
cells.append(md("c18-md-nnls",
"## Step 5: Constrained Optimization - NNLS\n",
"\n",
"**Model:** I_exp(v) = sum_i w_i * I_i(v), where w_i >= 0\n",
"\n",
"**Method:** Non-Negative Least Squares (scipy.optimize.nnls; Lawson & Hanson, 1974)\n",
"- Minimizes ||I_exp - B*w||^2 subject to w >= 0\n",
"- Guarantees global optimum (convex quadratic, polyhedral constraints)\n",
"- No initial guess required; no risk of local minima\n",
"- Weights are post-normalized to sum=1 for compositional interpretation\n",
"\n",
"**Metrics:** RMSE, R^2, Pearson r are all computed on the same normalized [0,1]\n",
"arrays on the same shared grid used for fitting — no array mismatch.\n",
))

cells.append(code("c19-nnls",
...
cells.append(code("c20-slides3-4",
...
# ─────────────────────────────────────────────────────────────────────────────
# CELL 21 — Residual Analysis (NEW)
# ─────────────────────────────────────────────────────────────────────────────
cells.append(code("c21-residuals",
...
# ─────────────────────────────────────────────────────────────────────────────
# CELL 22-23 — Slide 5 + Interpretation
# ─────────────────────────────────────────────────────────────────────────────
cells.append(md("c22-md-s5","## Step 6: Dominant Contributors and Regional Interpretation\n"))

cells.append(code("c23-slide5",
...
# ─────────────────────────────────────────────────────────────────────────────
# CELL 24-25 — Peak Detection (Adaptive)
# ─────────────────────────────────────────────────────────────────────────────
cells.append(md("c24-md-peaks",
"## Step 7: Peak Detection (Adaptive Prominence)\n",
"\n",
"Adaptive noise estimation per spectrum: local noise floor = MAD of the spectrum.\n",
"Prominence threshold is set relative to local noise, not a global fixed value.\n",
"This avoids false peaks from residual noise in quiet spectral regions,\n",
"and avoids missing peaks in molecule-specific active regions.\n",
))

cells.append(code("c25-peaks",
...
# ─────────────────────────────────────────────────────────────────────────────
# CELL 26 — Metrics Table
# ─────────────────────────────────────────────────────────────────────────────
cells.append(code("c26-metrics",
"metrics_rows = [\n",
...
# ─────────────────────────────────────────────────────────────────────────────
cells.append(md("c27-md-summary","## Step 8: Final Scientific Summary\n"))

cells.append(code("c28-summary",
